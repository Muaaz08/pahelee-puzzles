...
const canShowEmulator = import.meta.env.DEV && typeof window !== "undefined";

export function DeviceEmulator({ children }: { children: React.ReactNode }) {
  const [on, setOn] = useState<boolean>(() => {
    if (!canShowEmulator) return false;
    try {
  ...
  if (!canShowEmulator) return <>{children}</>;
